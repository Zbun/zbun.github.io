;(function($) {
    ...
    $.fn.cityList = function(options) {
        var indexProv = 0,
            indexCity = 0,
            $city = $($(this).nextAll('select')[0]) || '',
            $downtown = $($(this).nextAll('select')[1]) || '';
        if (options) {
          if(typeof options==='object'){
            $city = $(options['city']) || $city;
            $downtown = $(options['downtown']) || $downtown;
          }
        }
        var l = areas.length,i=0,
            htmls = '';
        while (i<l) {
            htmls += '<option' + ' data-index=' + i + '>' + areas[i].name + '</option>';
            i++;
        }
        this.html(htmls);
        this.change(function() {
            var $t = $(this);
            indexProv = $t.find(':selected').data('index') == 0 ? 0 : $t.find(':selected').data('index');
            var cityCur = areas[indexProv].city;
            var htmlCity = '',
                l = cityCur.length;
            for (i = 0; i < l; i++) {
                htmlCity += '<option' + ' data-index=' + i + '>' + cityCur[i].name + '</option>';
            }
            $city.html(htmlCity);

            indexCity = $city.find(':selected').data('index') == 0 ? 0 : $city.find(':selected').data('index');
            var dt = areas[indexProv]['city'][indexCity]['area'],
                lD = dt.length;
            var htmlDowntown = '';
            // var htmls='',l=areas[i]['city'].length;
            for (i = 0; i < lD; i++) {
                htmlDowntown += '<option' + ' data-index=' + lD + '>' + dt[i] + '</option>';
            }
            $downtown.html(htmlDowntown);
        ...
        $city.change(function() {
            var $t = $(this);
            indexCity = $t.find(':selected').data('index') == 0 ? 0 : $t.find(':selected').data('index');
            var dt = areas[indexProv]['city'][indexCity]['area'],
                l = dt.length;
            var htmlDowntown = '';
            // var htmls='',l=areas[i]['city'].length;
            for (i = 0; i < l; i++) {
                htmlDowntown += '<option' + ' data-index=' + i + '>' + dt[i] + '</option>';
            }
            $downtown.html(htmlDowntown);
        })
        $city.change();
